import { SerialMode, SerialTransformer } from './serialtransformer';
import { assert, stringToBytes, mergeByteChunks } from '../bitt101/utils';
...
export class BTSerial {

  private port: SerialPort;
  private options: SerialOptions;
  private readTransformer: SerialTransformer;
  ...
  async readBytes(numBytes = 0): Promise<Uint8Array> {
    ...
    this.isReading = true;
    try {
      this.readTransformer.setMode(SerialMode.Bytes);
  ...
  async readLine(): Promise<string> {
    ...
    this.isReading = true;
    try {
      this.readTransformer.setMode(SerialMode.Lines);
